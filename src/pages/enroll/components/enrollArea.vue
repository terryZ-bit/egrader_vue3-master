<template>
  <div id="EnrollArea">
    <h1 style="text-align: left; margin-left: 20px; margin-top: 40px">
      欢迎注册!
    </h1>
    ...
    <div class="enroll-input-area" style="margin-top: 40px">
      ...
      >
        ...
        <t-form-item name="name" class="enroll-form-item" label="昵称">
          <t-input v-model="formData.name" clearable placeholder="请输入昵称">
          </t-input>
        </t-form-item>
...
export default {
  ...
  methods: {
    async onSubmit(validateResult) {
      try {
        if (validateResult === true) {
          ...
          await MessagePlugin.error('输入格式错误!')
        }
      } catch (e) {
        console.log(e)
      }
    ...
    sendVerifyCode() {
      if (this.formData.value.email.length < 1) {
      ...
      getVerificationCode(this.formData.email)
        .then((resp) => {
          console.log(resp)
