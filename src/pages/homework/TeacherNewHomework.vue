...
const filesUpload = async (file) => {
  ...
  console.log(file[0])
  await getTeacherHomeworkOssPaths(chooseRole.value.roleId, newHomeworkId.value, [file[0].name]).then(async (resp) => {
    // @ts-ignore
  ...
  await oss
    .put(fileInfo.oss_path + fileInfo.file_name, file[0].raw)
    .then(async () => {
  ...
  return uploadFlag ? requestSuccessMethod(file) : requestFailMethod(file)
}
